 ...
 * ----------------------------------------------------------------
 *
 * Copyright (C) 1999-2005 Mojave Group, Cornell University
 *
...
struct
   ...
   type 'a t =
      { mutable array_shift : int;
        ...
        mutable array_buckets : 'a Weak.t array
      }
   ...
   let expand info i =
      ...
      let length = 1 lsl log in
         if log <= max_shift then
            let new_array = Weak.create length in
            let old_array = buckets.(0) in
               Weak.blit old_array 0 new_array 0 (Weak.length old_array);
            ...
            begin
               ...
               (if info.array_length < max_length then
                   let new_array = Weak.create max_length in
                   let old_array = buckets.(0) in
                      Weak.blit old_array 0 new_array 0 (Weak.length old_array);
                      buckets.(0) <- new_array);
               ...
               let old_length = Array.length buckets in
                  ...
                  for i = old_length to pred count do
                     new_buckets.(i) <- Weak.create max_length
                  done;
   ...
   let set info i x =
      if i > info.array_length then
         expand info i;
      Weak.set info.array_buckets.(i lsr info.array_shift) (i land info.array_mask) (Some x)
   ...
   let get info i =
      ...
      Weak.get info.array_buckets.(i lsr info.array_shift) (i land info.array_mask)
end
