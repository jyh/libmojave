 ...
 * ----------------------------------------------------------------
 *
 * Copyright (C) 1998, 1999, 2001 Jason Hickey, Caltech
 *
...
and 'tag formatted_info =
   { formatted_commands : 'tag print_command list;
     formatted_breaks : bool array;     (* Lm_flags for breaks that were taken *)
 ...
 *)
let empty_info =
   Unformatted { unformatted_commands = []; unformatted_index = 0 }
         ...
         let rec flush = function
          ...
          | { formatting_commands = commands;
              ...
              formatting_buf = buf
               ...
               flush tl
          | [] ->
               raise (Invalid_argument "Mc_rformat.flush_formatting")
         in
...
let push_zone buf tag =
   ...
   let _ =
      match stack.formatting_stack with
       ...
       | [] ->
            raise (Invalid_argument "Mc_rformat.push_zone")
   in
   let info =
      { formatting_commands = [];
        formatting_index = 0;
...
let get_formatting_head buf =
   match (get_formatting_stack buf).formatting_stack with
      head :: _ ->
         head
    | [] ->
         raise (Invalid_argument "Mc_rformat.get_formatting_head")
...
let rec format_string buf s =
   try
       let i = Lm_string_util.strchr s '\n' in
          push_command buf (Text (i, Lm_string_util.sub "Mc_rformat.format_string" s 0 i));
          format_newline buf;
          let l = (String.length s) - i - 1 in
             if l > 0 then
                format_string buf (Lm_string_util.sub "Mc_rformat.format_string" s (i + 1) l)
   with
      Not_found ->
...
let get_unformatted buf =
   match buf.buf_info with
    ...
    | Formatting _ ->
         raise (Invalid_argument "Mc_rformat.get_unformatted")
...
let get_formatted buf =
   match buf.buf_info with
    ...
    | Formatting _ ->
         raise (Invalid_argument "Mc_rformat.get_formatted")

(*
 * Breaks are never taken in a linear zone.
 *)
let rec search_lzone buf lmargin rmargin col maxx search =
   if debug debug_rformat then
      eprintf "Mc_rformat.search_lzone%t" eflush;
...
and search_tzone buf lmargin rmargin col maxx breaks search =
   if debug debug_rformat then
      eprintf "Mc_rformat.search_tzone%t" eflush;
...
and search_zone buf lmargin rmargin col maxx breaks search =
   if debug debug_rformat then
      eprintf "Mc_rformat.search_zone%t" eflush;
   match buf.buf_tag with
      LZoneTag ->
...
let compute_breaks buf width =
   if debug debug_rformat then
      eprintf "Mc_rformat.compute_breaks%t" eflush;
...
let refresh_breaks buf =
   ...
   match buf.buf_info with
    ...
    | Unformatted _
    | Formatting _ ->
         raise (Invalid_argument "Mc_rformat.compute_breaks")

(*
 * "tab" to a position on the next line.
 *)
let tab printer pos =
   printer ("\n" ^ (Lm_string_util.make "Mc_rformat.tab" pos ' '))
...
let rec print_lzone buf rmargin col printer =
   if debug debug_rformat then
      eprintf "Mc_rformat.print_lzone%t" eflush;
...
and print_tzone buf rmargin col printer =
   if debug debug_rformat then
      eprintf "Mc_rformat.print_tzone%t" eflush;
...
and print_zone buf rmargin col printer linear =
   if debug debug_rformat then
      eprintf "Mc_rformat.print_zone%t" eflush;
   match buf.buf_tag with
      LZoneTag ->
...
let print_buf buf rmargin printer =
   if debug debug_rformat then
      eprintf "Mc_rformat.print_buf%t" eflush;
 ...
 * Generic printer.
 *)
let print_to_printer rmargin buf printer =
 ...
 *)
let print_to_channel rmargin buf outx =
   print_to_printer rmargin buf (make_channel_printer outx)

let print_to_string rmargin buf =
   let get_string, printer = make_string_printer () in
      print_to_printer rmargin buf printer;
      get_string ()

(*
 * Print to HTML.
 *)
let print_to_html rmargin buf outx =
   let get_info, printer = make_html_printer outx in
      print_to_printer rmargin buf printer;
      get_info ()
 ...
 * TeX formatting.
 *)
let print_to_tex rmargin buf outx =
   let get_info, printer = make_tex_printer outx in
      print_to_printer rmargin buf printer
